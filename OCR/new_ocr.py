...
# Recommended pacing knobs (fixes “fast video” + unstable timing)
TARGET_DISPLAY_FPS = 30.0      # cap UI refresh
TARGET_DEBUG_FPS = 30.0        # fps encoded into debug mp4
OCR_HZ = 2.0                   # run OCR this many times per second (GPU or CPU)

# Recommended camera defaults (webcam only)
FORCE_WEBCAM_RES = True
...
def init_easyocr():
    ...
    # Warmup helps smooth first inference
    try:
...
# ----------------------------
# Main (rewritten with recommended pacing + stable recording)
# ----------------------------
def main():
    ...
    if VIDEO_PATH == "0" or VIDEO_PATH == 0:
        ...
        # Reduce internal buffering (prevents “draining” backlog too fast)
        ...
        target_video_fps = TARGET_DISPLAY_FPS  # for pacing + debug encode
    else:
        ...
        target_video_fps = float(file_fps) if file_fps and file_fps > 1 else TARGET_DISPLAY_FPS

    if not cap.isOpened():
        print("Error: Could not open video source")
        return

    # Encode debug video at the SAME fps we pace the loop
    debug_fps = float(TARGET_DEBUG_FPS)
    if VIDEO_PATH != "0" and VIDEO_PATH != 0:
        debug_fps = target_video_fps
    ...
    print(f"Target pacing FPS: {target_video_fps:.1f}")
    print(f"Debug encode FPS: {debug_fps:.1f}")
    if total_frames > 0:
    ...
    # --- Timing controls ---
    frame_period = 1.0 / max(1.0, float(target_video_fps))
    next_frame_time = time.time()

    display_period = 1.0 / max(1.0, float(TARGET_DISPLAY_FPS))
    next_display_time = time.time()

    ocr_period = 1.0 / max(0.1, float(OCR_HZ))
    next_ocr_time = time.time()
    ...
    try:
        while True:
            ...
            # --- Pace capture loop so you don't “race” and create fast videos ---
            next_frame_time += frame_period
            sleep_t = next_frame_time - time.time()
            if sleep_t > 0:
                time.sleep(sleep_t)
            now = time.time()

            # --- OCR throttled by time (stable even if camera FPS is weird) ---
            if now >= next_ocr_time:
                next_ocr_time = now + ocr_period
            ...
            # --- Build overlay frame (same as your original, but with stable sizing) ---
            ...
            # IMPORTANT: keep consistent output size (prevents “text smaller” surprises)
            # If you want a max width, force the CAMERA to 1280 instead of resizing here.
            # So we REMOVE the old "if disp.shape[1] > 1280: resize" behavior.

            # --- Init writer with the final disp size ---
            if SAVE_DEBUG_VIDEO and not writer_inited:
                h, w = disp.shape[:2]
                fourcc = cv2.VideoWriter_fourcc(*"mp4v")
                writer = cv2.VideoWriter(DEBUG_VIDEO_PATH, fourcc, float(debug_fps), (w, h))
                writer_inited = True
                print(f"[debug] Writing overlay video to: {DEBUG_VIDEO_PATH} @ {debug_fps:.1f} fps, size=({w}x{h})")

            if SAVE_DEBUG_VIDEO and writer is not None:
                writer.write(disp)

            # --- Display throttle (keeps UI smooth) ---
            if not HEADLESS:
                if time.time() >= next_display_time:
                    next_display_time = time.time() + display_period
